---
title: 'Migrating Legacy Rich Text Editor to ReactJS'
summary: 'How we migrated our legacy rich text editor to ReactJS. Originally our text editor at Juniper Square was using JQuery and RequireJS under the hood. Since ReactJS is based off an object-oriented pattern which uses a virtual DOM to representits state, we were presented with many challenges including how to save editor content in a valid data format (I.e. save the object that is a representation of the editor content vs save the HTML output), how to ensure compatibility between legacy content saved in the editor and new content (it hasd to be backwards compatible), and how to optimize the loading of the package so that we could share dependencies and only load the legacy editor modules and the modern editors'
tags: texteditor, richtextediting, architecture
---
